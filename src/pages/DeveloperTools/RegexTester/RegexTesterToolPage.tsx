...
export function RegexTesterToolPage() {
  ...
  function handleTest() {
    ...
    showToast(
      result.matches.length ? "success" : "info",
      result.matches.length ? "Matches found." : "No matches found.",
    );
  }
    ...
    <ToolPageLayout
      ...
      overview={
        <div className="space-y-3">
          <p>
            Regular expressions are patterns used to search, validate and
            extract text.
          </p>
          <p>
            They are commonly used in APIs, validation rules, log analysis and
            data processing.
          </p>
        </div>
      }
        ...
        <div className="space-y-5">
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <HelpTooltip
                title="Regex"
                description="Enter a regular expression pattern."
                exampleInput="\\d+"
                exampleOutput="Number matches"
              />
            </div>
            <TextInput
              id="regex-pattern"
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <HelpTooltip
                title="Flags"
                description="Configure regex matching behavior."
                exampleInput="g i"
                exampleOutput="Global, case-insensitive matching"
              />
            </div>
            <div
              className="flex flex-wrap gap-2"
              role="group"
              aria-labelledby="regex-flags-label"
            >
              {regexFlags.map((flag) => (
                <button
                  key={flag.value}
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-semibold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-600 dark:focus-visible:outline-cyan-400 ${flags.includes(flag.value) ? "border-cyan-600 bg-cyan-50 text-cyan-800 ring-1 ring-cyan-600 dark:border-cyan-500 dark:bg-cyan-950 dark:text-cyan-200 dark:ring-cyan-500" : "border-gray-200 bg-white text-gray-700 hover:border-cyan-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:border-cyan-700"}`}
                >
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <HelpTooltip
                title="Test Text"
                description="Text content used for matching."
                exampleInput="hello@example.com"
                exampleOutput="Email match"
              />
            </div>
            <Textarea
              id="regex-text"
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Button color="blue" onClick={handleTest}>
              Test Regex
            </Button>
            <Button color="gray" onClick={clearAll}>
              Clear
            </Button>
            <Button color="light" onClick={() => void copyMatches()}>
              Copy Matches
            </Button>
          </div>
          <section>
            <Button
              ...
              aria-controls="common-regex-patterns"
            >
              ...
              >
                {commonPatterns.map((item) => (
                  <button
                    ...
                    className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 transition hover:border-cyan-300 hover:text-cyan-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:focus-visible:outline-cyan-400"
                  >
                    {item.label}
                  </button>
                ))}
      ...
      outputs={
        <div className="space-y-5">
          <div className="flex items-center gap-2">
            ...
            <HelpTooltip
              title="Results"
              description="Detected matches and match information."
              exampleInput="Match Count"
              exampleOutput="3 matches"
            />
          </div>
          ...
          <div className="grid gap-3">
            {matches.map((match, index) => (
              <div
                ...
                className="rounded-lg bg-gray-50 p-3 dark:bg-gray-950"
              >
                ...
                <Badge color="info" className="mt-2 w-fit">
                  Position {match.index}-{match.endIndex}
                </Badge>
              </div>
            ))}
          </div>
...
function HighlightedText({
  ...
  matches.forEach((match, index) => {
    ...
    parts.push(
      <mark
        ...
        className="rounded bg-yellow-200 px-1 text-gray-950"
      >
        {text.slice(match.index, match.endIndex)}
      </mark>,
    );
  ...
  return (
    <pre className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm whitespace-pre-wrap text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
      {parts}
    </pre>
  );
}
